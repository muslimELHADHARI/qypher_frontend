...
import { AttackVectorsDisplay } from "@/components/AttackVectorsDisplay";
  ...
  return (
    <div className="space-y-6 page-fade-in pb-10">
        ...
        {[
          ...
          <div key={idx} className="enterprise-card p-5 flex flex-col justify-between hover:border-primary/30 transition-colors group">
            <div className="flex items-start justify-between mb-4">
              ...
              <div className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full ${kpi.up ? "text-emerald-500 bg-emerald-500/10" : "text-rose-500 bg-rose-500/10"
                }`}>
                {kpi.trend}
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Threat Distribution */}
        <AttackVectorsDisplay
          data={distributionData}
          totalVectors={stats?.attackTypes?.length || 0}
          stats={stats}
        />
        {/* Severity Timeline */}
              ...
              {combinedEvents.map((evt) => (
                <tr key={evt.id} className="hover:bg-muted/5 transition-colors group cursor-default">
                  ...
                  <td className="py-3 px-4 text-center">
                    <div className={`inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-sm border shadow-sm ${evt.severity === "CRITICAL" ? "text-rose-500 border-rose-500/20 bg-rose-500/10" :
